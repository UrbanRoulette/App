...
//		var start_hour = convert_date_to_hour_integer(start_date);
	...
	update_subsequent_activities_dates = function(c,fill,sign){
		for(j=results_of_activities.length - c; j < results_of_activities.length; j++){
			var act = results_of_activities[j];
			//
	...
	remove_last_activity_from_results = function(){
		var last_a = results_of_activities[results_of_activities.length - 1];
		if(activities_locked_ids.indexOf(last_a._id) > -1){
			activities_locked_idx -= 1;
		}
...
Meteor.methods({
	...
	get_activities_results: function(center,radius,date,profile,timezoneOffset){
		...
		//activities_locked = array.sort(function(a,b){return ((a.start_time).getTime() - (b.start_time).getTime());});
		...
		end_points.push(end_roulette_date);
...
//		var profile = ["gratuit", "cheap", "exterieur", "curieux", "couple", "solo", "potes", "prestige"];
		...
		activities_locked_idx = 0; //Must be defined globally
		...
		do {
			...
			//Adds the locked_activity if it's relevant
			while(activities_locked.length >= activities_locked_idx + 1){

				var activity_locked = activities_locked[activities_locked_idx];
				...
				if(activity_locked.start_date.getTime() < date_cursor.getTime()){						
					...
					if(diff_last < 0)
						activity_locked.end_date = new Date(activity_locked.end_date.getTime() - diff_last*ms_in_min);
...
//					last_doc_query = {$where: function(){return ((End - this.opening_hours.end_minus_last_min) <= this.last.last_min)}};
			...
			var ind;
			if (eatingHours.indexOf(hour) > -1 && types_excluded.indexOf(type_considered) === -1 && (profile.indexOf('gratuit') === -1 || profile.length > 1))
...
*/	//						requiresun: requiresun,
			...
			//Deal if activity is not defined
			...
			var end_point = end_points[activities_locked_idx];
			...
			if(diff_beg > 0){
				...
				while (diff_beg > 0){ //Need to flex up in this case (because activity opens after date_cursor)
					...
					if(c > 0){
						update_subsequent_activities_dates(c,fill,1);
					}
